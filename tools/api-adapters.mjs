...
export const API_AGENTS = ['openai', 'gemini', 'ollama'];
...
export function apiConfiguration(agent, env = process.env) {
  ...
  return { endpoint, key: key || null, keyName, configured: agent === 'ollama' || Boolean(key) };
}

export function apiDoctor(agent, env = process.env) {
  const config = apiConfiguration(agent, env);
  return { agent, status: config.configured ? 'configured' : 'unconfigured', configured: config.configured, liveVerified: false, authentication: agent === 'ollama' ? 'Optional OLLAMA_API_KEY; service/model availability not checked' : `${config.keyName}${agent === 'gemini' ? ' or GOOGLE_API_KEY' : ''} ${config.configured ? 'present' : 'required'}`, endpoint: config.endpoint, tools: [], mode: 'single-request text/files', note: 'No network request made; run a bounded smoke job to verify access and model support.' };
}
...
export async function executeApi(job, context, { fetchImpl = fetch, env = process.env, signal, cancelled = async () => false } = {}) {
  ...
  try {
    ...
    const credentials = ['OPENAI_API_KEY', 'GEMINI_API_KEY', 'GOOGLE_API_KEY', 'OLLAMA_API_KEY'].map(key => env[key]).filter(value => typeof value === 'string' && value.length >= 8);
