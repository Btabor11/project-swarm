...
Four adapters are available: `claude`, `openai`, `gemini`, and `ollama`. API jobs require an explicit model and accept UTF-8 text only; they have no tools and return complete declared file contents as validated JSON. Claude may omit `model` to use its CLI default. A model string is not proof of availability. API adapters have deterministic contract tests; do not claim live verification without an actual successful exchange. A CLI worker does not open a visible terminal window.
