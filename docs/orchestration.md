...
Native agents use the host's scheduler; CLI/API jobs use this runner and their selected providers. A limit on native agents does not automatically impose the same limit on external jobs. Shared machine resources, provider rate limits, account access, and host execution rules can still constrain both. Do not claim the host is the only available execution path, or that external capacity is unlimited, from a native slot count alone.

Before choosing a path, inspect installed adapters and the selected provider's configuration. `doctor` proves neither authentication nor remaining quota. Use the required bounded read-only and writing smoke exchanges before a substantial first batch, inspect the actual provider/model response, and preflight output ownership across all active runs and native agents. A successful smoke establishes that those exchanges worked at that time; it does not certify a larger concurrency level. Stop on actual access or execution restrictions rather than weakening them. Extra capacity is useful only for ready, independent deliverables; do not create idle managers or jobs to fill slots.

Keep a timestamped activity ledger with native running agents, external running jobs by run/provider, queued jobs, and completed work awaiting review. State whether the coordinator is included. Combine native and external running counts only for the same observation window and avoid double-counting; summing peaks from different times does not produce an observed combined peak. The runner's `monitor` covers its own run, so the coordinator must obtain native and other-run state separately. Recorded running state is not proof of productive work and may be stale after a crash.

Report completed-job provider usage and reported cost with their scope and missing fields. In-flight or unreported usage is not included in a completed-job subtotal. Keep providers' token fields distinct, and label list-price or estimated cost as reported estimates rather than actual account charges. Run records do not measure subscription allowance, remaining account quota, credits, or the eventual invoice; those require separate current account evidence. Neither a higher worker count nor a larger usage subtotal proves faster delivery.
